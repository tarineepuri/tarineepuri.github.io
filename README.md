...
<head>
  ...
  <title>Tarinee Puri — CV</title>
  <!--
    Ready-to-use single-file CV website.
    How to use:
      1. Replace placeholder text (NAME, ROLE, EXPERIENCE, etc.) with your real content.
      2. Replace "avatar.jpg" with your photo in the same repo (or change the path).
      3. Upload a resume as "resume.pdf" in the repo root to enable the "Download CV" button.
      4. Commit this file as index.html to your GitHub repo. To publish as a user page use a repo named <your-username>.github.io.
  -->
  <style>
    :root{
      --bg:#0f1724;
    ...
    *{box-sizing:border-box}
    html,body{height:100%;margin:0;background:linear-gradient(180deg,#071027 0%, #0e1722 100%);color:#e6eef8}
    a{color:var(--accent);text-decoration:none}
    .wrap{max-width:1000px;margin:36px auto;padding:28px;background:linear-gradient(180deg, rgba(255,255,255,0.02), rgba(255,255,255,0.01));border-radius:12px;box-shadow:0 8px 30px rgba(2,6,23,0.6);display:grid;grid-template-columns:280px 1fr;gap:26px}

    /* Left column */
    .profile{padding:18px;background:var(--card);border-radius:10px}
    .avatar{width:100%;border-radius:8px;display:block}
    ...
    .btn{display:inline-block;padding:8px 12px;border-radius:8px;background:linear-gradient(90deg,var(--accent),#5b21b6);color:white;font-weight:600;margin-top:12px}

    /* Right column */
    .card{padding:18px;background:var(--glass);border-radius:10px;margin-bottom:16px}
    .section-title{display:flex;justify-content:space-between;align-items:center;margin-bottom:8px}
    .section-title h2{margin:0;font-size:16px}
    p{margin:8px 0;color:#cfe3ff}
    ...
    .skill .bar > i{display:block;height:100%;background:linear-gradient(90deg,var(--accent),#b084ff);width:0%}

    .projects{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px}
    .project{padding:10px;background:rgba(255,255,255,0.02);border-radius:8px}
...
<body>
  <div class="wrap" role="main">
    ...
    <section>
      ...
      <article class="card" aria-labelledby="skills">
